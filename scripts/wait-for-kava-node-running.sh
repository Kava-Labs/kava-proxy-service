...
until kava status -n tcp://localhost:"${KAVA_HOST_COSMOS_RPC_PORT}" | jq
do
    echo "waiting for kava service to be running"
    sleep 1
done

BLOCK_NUMBER=$(kava status -n tcp://localhost:"${KAVA_HOST_COSMOS_RPC_PORT}" | jq .sync_info.latest_block_height)
